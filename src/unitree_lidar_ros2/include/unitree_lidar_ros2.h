...
void UnitreeLidarSDKNode::timer_callback()
{
    ...
    if (result == LIDAR_IMU_DATA_PACKET_TYPE)
    {
        ...
        if (lsdk_->getImuData(imu))
        {
            ...
            transformStamped.transform.rotation.x = -imu.quaternion[1]; // changed for inversed lidar 
            transformStamped.transform.rotation.y = -imu.quaternion[2];// changed for inversed lidar 
